...
void Server::getResponse(Client & client)
{
    ...
    try
    {
        ...
        else if (url.find("/Cookies/") != std::string::npos)
        {
            ...
            if (cookie != "not found")
            {
                ...
                while (getline(iss, parser, ';'))
                {
                    ...
                    if (parser.find("lang=") != std::string::npos)
                    {
                        ...
                        {
                            client.setPathFile(Path("/home/luis/proyects/webserv/html/CookiesHTTPEng.html"));
                        }
                        else
                            client.setPathFile(Path("/home/luis/proyects/webserv/html/CookiesHTTPEsp.html"));
                    }
            ...
            {
                client.setDefaultHttpResponse(OK);
                client.setPathFile(Path("/home/luis/proyects/webserv/html" + client.getURL().substr(client.getURL().find_last_of("/"))));
            }
        ...
        {
            client.setDefaultHttpResponse(OK);
            client.setPathFile(Path("/home/luis/proyects/webserv/html/index.html"));
            client.setResponseType(FILE_OBJ);
        }
        else if (url == "/nolen.py")
        {
            client.setDefaultHttpResponse(OK);
            client.setPathFile(Path("/home/luis/proyects/webserv/CGI/nolen.py"));
